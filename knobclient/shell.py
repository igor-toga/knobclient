...
"""
Command-line interface to the Knob API.
"""
...
from knobclient import client as knob_client
...
class KnobShell(object):
    ...
    def _append_global_identity_args(self, parser):
        ...
        parser.add_argument(
            ...
            help=_('Explicitly allow knobclient to perform '
    ...
    def get_base_parser(self):
        ...
        parser.add_argument('--version',
                            action='version',
                            version=knobclient.__version__,
                            help=_("Shows the client version and exits."))

        parser.add_argument('-d', '--debug',
                            default=bool(utils.env('KNOBCLIENT_DEBUG')),
                            action='store_true',
                            help=_('Defaults to %(value)s.') % {
                                'value': 'env[KNOBCLIENT_DEBUG]'
                            })
        ...
        # os-no-client-auth tells knobclient to use token, instead of
        # env[OS_AUTH_URL]
        ...
        parser.add_argument('--knob-url',
                            default=utils.env('KNOB_URL'),
                            help=_('Defaults to %(value)s.') % {
                                'value': 'env[KNOB_URL]'
                            })

        parser.add_argument('--knob_url',
                            help=argparse.SUPPRESS)

        parser.add_argument('--knob-api-version',
                            default=utils.env('KNOB_API_VERSION', default='1'),
                            help=_('Defaults to %(value)s or 1.') % {
                                'value': 'env[KNOB_API_VERSION]'
                            })

        parser.add_argument('--knob_api_version',
                            help=argparse.SUPPRESS)
        ...
        parser.add_argument('--include-password',
                            default=bool(utils.env('KNOB_INCLUDE_PASSWORD')),
                            action='store_true',
                            help=_('Send %(arg1)s and %(arg2)s to knob.') % {
                                'arg1': 'os-username',
                                'arg2': 'os-password'
                            })
        ...
        if osprofiler_profiler:
            parser.add_argument(
                ...
                help=_('HMAC key to use for encrypting context data '
                       ...
                       'configured in osprofiler middleware in knob, '
                       'it is specified in the paste configuration '
                       '(/etc/knob/api-paste.ini). Without the key, '
    ...
    def get_subcommand_parser(self, version):
        ...
        submodule = importutils.import_versioned_module('knobclient',
                                                        version, 'shell')
    ...
    def _get_keystone_session(self, **kwargs):
        ...
        if verify is None:
            if insecure:
                ...
                # knobclient.common.http.get_system_ca_fle()?
    ...
    def main(self, argv):
        ...
        api_version = options.knob_api_version
        ...
        if args.os_no_client_auth:
            if not args.knob_url:
                raise exc.CommandError(_("If you specify --os-no-client-auth "
                                         "you must also specify a Knob API "
                                         "URL via either --knob-url or "
                                         "env[KNOB_URL]"))
        else:
        ...
        if args.os_no_client_auth:
            # Do not use session since no_client_auth means using knob to
            # to authenticate
            kwargs = {
            ...
            kwargs = {
                ...
                'password': args.os_password,
                'include_pass': args.include_password,
                'endpoint_override': args.knob_url,
            }

        client = knob_client.Client(api_version, **kwargs)
    ...
    def do_bash_completion(self, args):
        ...
        The knob.bash_completion script doesn't have to hard code them.
        """
        commands = set()
        options = set()
...
def main(args=None):
    try:
        ...
        print(_("... terminating knob client"), file=sys.stderr)
        sys.exit(130)
