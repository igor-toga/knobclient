...
from knobclient.i18n import _LW
...
class Knob(app.App):
    ...
    def _get_endpoint_filter_kwargs(self, args):
        endpoint_filter_keys = ('interface', 'service_type', 'service_name',
                                'knob_api_version', 'region_name')
        kwargs = dict((key, getattr(args, key)) for key in endpoint_filter_keys
                      if getattr(args, key, None))
        if 'knob_api_version' in kwargs:
            kwargs['version'] = kwargs.pop('knob_api_version')
        return kwargs
    ...
    def build_option_parser(self, description, version, argparse_kwargs=None):
        ...
        """
        parser = super(Knob, self).build_option_parser(
        ...
        parser.add_argument('--endpoint', '-E',
                            metavar='<knob-url>',
                            default=utils.env('KNOB_ENDPOINT'),
                            help='Defaults to env[KNOB_ENDPOINT].')
        parser.add_argument('--interface',
                            metavar='<knob-interface>',
                            default=utils.env('KNOB_INTERFACE'),
                            help='Defaults to env[KNOB_INTERFACE].')
        parser.add_argument('--service-type',
                            metavar='<knob-service-type>',
                            default=utils.env('KNOB_SERVICE_TYPE'),
                            help='Defaults to env[KNOB_SERVICE_TYPE].')
        parser.add_argument('--service-name',
                            metavar='<knob-service-name>',
                            default=utils.env('KNOB_SERVICE_NAME'),
                            help='Defaults to env[KNOB_SERVICE_NAME].')
        parser.add_argument('--region-name',
                            metavar='<knob-region-name>',
                            default=utils.env('KNOB_REGION_NAME'),
                            help='Defaults to env[KNOB_REGION_NAME].')
        parser.add_argument('--knob-api-version',
                            metavar='<knob-api-version>',
                            default=utils.env('KNOB_API_VERSION'),
                            help='Defaults to env[KNOB_API_VERSION].')
        parser.epilog = ('See "knob help COMMAND" for help '
    ...
    def run(self, argv):
        ...
        if not argv:
            ...
            return 1
        return super(Knob, self).run(argv)


def main(argv=sys.argv[1:]):
    logging.basicConfig()
    LOG.warning(_LW("This Knob CLI interface has been deprecated and "
                    "will be removed in the O release. Please use the "
                    "openstack unified client instead."))
    knob_app = Knob()
    return knob_app.run(argv)
